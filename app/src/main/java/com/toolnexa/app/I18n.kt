...
object I18n {
    ...
    private val extraTranslations =
        mapOf(
            ...
            "O resultado foi criado pelo motor neural Basic Pitch, com transcrição polifónica e informação de pitch bend. O modelo é local e a análise não depende de uma API externa." to mapOf("en" to "The result was created by the Basic Pitch neural engine with polyphonic transcription and pitch-bend data. The model is bundled locally and analysis does not depend on an external API.", "es" to "El resultado fue creado por el motor neuronal Basic Pitch con transcripción polifónica y datos de pitch bend. El modelo está incluido localmente y el análisis no depende de una API externa.", "fr" to "Le résultat a été créé par le moteur neuronal Basic Pitch avec transcription polyphonique et données de pitch bend. Le modèle est intégré localement et l’analyse ne dépend pas d’une API externe.", "ar" to "تم إنشاء النتيجة بواسطة محرك Basic Pitch العصبي مع تفريغ متعدد النغمات وبيانات انحناء النغمة. النموذج مضمّن محليًا ولا يعتمد التحليل على واجهة API خارجية."),
